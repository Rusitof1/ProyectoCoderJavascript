...
function actualizarStorage() {
  ...
  let sumTotal = carrito.reduce(
    (total, clickedItem) => total + clickedItem.precio,
    0
  );
...
//Search bar
document
  .getElementById("search__item__button")
  .addEventListener("click", function (e) {
    e.preventDefault();
    let buscar = document
      .getElementById("search__item__input")
      .value.toLowerCase();
    const searchedItem = catalogo.filter((prod) =>
    ...
    if (searchedItem.length > 0) {
      ...
      lista.innerHTML = `<div class="alert__no__results">No existen resultados para su busqueda</div>`;
      setTimeout(() => {
        lista.innerHTML = "";
        fetchData();
      }, 2000);
  ...
  if (carritoStorage.length >= 1) {
    ...
    if ((total = !null)) {
      ...
      if (sumTotalStorage > 0) {
        total.innerText = `Total $${sumTotalStorage}`;
      }
    ...
    //Eventos de botones
    document
      .getElementById("terminar__carrito")
      .addEventListener("click", () => {
        swalCheckout();
      });
  } else {
    lista.innerHTML = `<div class="alert__no__results">Aun no se han agregado productos al carrito</div>`;
    setTimeout(() => {
      lista.innerHTML = "";
      fetchData();
    }, 2000);
